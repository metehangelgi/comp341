...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for GhostPositionDistributions in livingGhostPositionDistributions:
            ghostPoses.append(GhostPositionDistributions.argMax())

        #Your agent should first find the most likely position of each remaining uncaptured ghost
        closestGhost = None
        closestGhostDist = 1000000000 #large distance to initialize
        ...
        bestAct = None
        shortestDist = 1000000000 #large distance to initialize
        for legalAction in legal:
            successorPos = Actions.getSuccessor(pacmanPosition,legalAction)
            successorDist = self.distancer.getDistance(successorPos,closestGhost)
            if successorDist < shortestDist:
                shortestDist = successorDist
                bestAct = legalAction
        return bestAct
